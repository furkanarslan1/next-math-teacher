...
export default async function Popular_courses() {
  const supabase = await createServerSupabase();

  // 1. popular_courses tablosu üzerinden ilişkili kurs verilerini çekiyoruz
  // 1. We retrieve related course data from the popular_courses table.
  const { data: popularData, error } = await supabase
    .from("popular_courses")
    .select("course_id, courses(id, title, slug, features, short_description)")
    .limit(4);
